...
def test_poison(helper,epoch,data_source,model,is_poison=False,visualize=True):
    ...
    for batch_id,batch in enumerate(data_iterator):
        ...
        if helper.params['type'] == 'text':
            ...
            output=model(data)
            total_loss+= nn.functional.cross_entropy(output, targets,reduction='sum').data.item()
...
def train(helper,epoch,train_data_sets,local_model, target_model, is_poison, last_weight_accumulator=None):
    ...
    for model_id in range(helper.params['no_models']):
        ...
        optimizer = torch.optim.SGD(model.parameters(), lr=helper.params['lr'],
                                    ...
                                    weight_decay=helper.params['decay'])  # 优化器SGD随机梯度下降，学习率，动量，权重占比大小
        #将模型设置为训练模式
        model.train()
        start_time = time.time()
        # 根据数据类型（文本或图像）进行不同的数据处理
        if helper.params['type'] == 'text':
        ...
        for name, data in model.state_dict().items():
            #### don't scale tied weights:不缩放绑定的权重
            if helper.params.get('tied', False) and name == 'decoder.weight' or '__' in name:
                continue
...
if __name__=='__main__':
    ...
    with open(f'./{args.params}') as f:
        params_loaded=yaml.load(f)
    current_time = datetime.datetime.now().strftime("%Y%m%d-%H.%M.%S")
    if params_loaded['type']=="image":
        helper=ImageHelper(current_time=current_time,params=params_loaded,name=params_loaded.get('name','image'))
    helper.load_data()
    helper.create_model()
    ...
    logger.info(f"跟随环境作图:  {helper.params['environment_name']}")
    participant_ids=range(len(helper.train_data))
    mean_acc=list()
    # poison_sentences在图像中用不到
    results = {'poison': list(), 'number_of_adversaries': helper.params['number_of_adversaries'],
               ...
               'baseline': helper.params['baseline']}  # 将结果封装
    ...
    for epoch in range(helper.start_epoch, helper.params['epochs'] + 1):
        ...
        if helper.params['random_compromise']:
            ...
            for pos,loader_id in enumerate(subset_data_chunks):
                if loader_id in helper.params['poison_loader_ids']:
            ...
            if epoch in helper.params['poison_epochs']:
                ...
                subset_data_chunks = random.sample(participant_ids[1:], helper.params['no_models'])  # 选择10个客户端
